...
void DecodedGeneticString::decode(GeneticString gs, LSystem LS, std::map<std::string, double> params) {
    ...
    for (int i = 0; i < gs.count(); i++) { // for each item of the main genetic-string
        ...
        if(num_components < params["max_comps"]-1) {  // if limit number of components has not been reasched
            ...
            if (LS.getAlphabet().count(current_gs_item->item) > 0) {  // if the item is a letter (component) in the alphabet
                ...
                if (root == NULL) { // if there are no components in the graph yet, creates root
                    ...
                    if (!command.empty()) { // if a command has been set for mounting the component
                        ...
                        if (component == "C" or
                            component == "B") { // if component is core or brick, having multilateral connection
                ...
                if (current_gs_item->item.substr(current_gs_item->item.size() - 1, 1) == "<") { // if it is a move-command
                    ...
                    command = current_gs_item->item.substr(current_gs_item->item.size() - 1,
                                                           1); // discovers the type of command, to be used with the next component to be mounted later on
