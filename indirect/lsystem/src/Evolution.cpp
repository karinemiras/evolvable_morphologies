 ...
 **/
void Evolution::addToArchive(std::vector<Genome *> *individuals,
                             double prob_add_archive, std::string path)
{
         ...
         i++)
    {
        ...
        if (prob(generator) < prob_add_archive)
        {
            ...
            this->archive->emplace(individuals->at(i)->getId(),
                                   gen_arc);
...
void Evolution::exportPop(int generation)
{
    ...
    { // for each genome in the population
        ...
        std::string filename = "/body_" + this->population->at(i)->getId()
                               + "_p1_" +
                               this->population->at(i)->getId_parent1() +
                               "_p2_" +
                               this->population->at(i)->getId_parent2() +
                               ".png";
        ...
        std::ifstream measures("../../experiments/" + this->experiment_name +
                               ...
                               std::to_string(generation_genome)
                               + "/measures" +
                               this->population->at(i)->getId() + ".txt");

        while (getline(measures,
                       line))
        {
...
double Evolution::compareParents(std::string idparent1, std::string idparent2)
{
                   ...
                   line))
    {
        ...
        dif += std::pow(std::stod(tokens[1]) - std::stod(tokens2[1]),
                        2);
    }
    dif = roundf(std::sqrt(dif) * 100) / 100;
 ...
 **/
void Evolution::measureIndividuals(int generation,
                                   ...
                                   std::string dirpath)
{
         ...
         i++)
    {
        ...
        Measures *m = new Measures(this->experiment_name,
                                   this->params);
        ...
        if (individuals->at(i)->getId_parent1() != "N")
        {
            ...
            double dif = this->compareIndividual(m,
                                                 individuals->at(i)->getId_parent1());
            differences_file << individuals->at(i)->getId() << " " << dif;

            dif = this->compareIndividual(m,
                                          individuals->at(i)->getId_parent2());
            differences_file << " " << dif;

            dif = this->compareParents(individuals->at(i)->getId_parent1(),
                                       individuals->at(i)->getId_parent2());
                           ...
                           std::vector<Genome *> *offspring)
{
    ...
    arma::mat compare(individuals_compare->at(0)->getMeasures().size(),
         ...
         i++)
    {
        ...
        arma::mat reference(individuals_reference->at(0)->getMeasures().size(),
                            1);
             ...
             ++j)
        {
            ...
            this->aux.logs(
                    ...
                    individuals_reference->at(i)->getId() + " has distance " +
...
void Evolution::compareIndividuals(int generation)
{
    ...
    for (const auto &it : this->morphological_grid_accumulated)
    {
        ...
        boost::split(tokens,
                     it.first,
                     boost::is_any_of("|"));
        std::vector<std::string> tokens2(tokens.begin(),
         ...
         i++)
    {
             ...
             j++)
        {
            ...
            double distance = 0;
            for (int d = 0;
                 d < this->population->at(0)->getMeasures().size();
                 d++)
            {

                distance += std::pow(points[i][d] - points[j][d],
                                     2);
            }
            // euclidean distance
    ...
    for (int i = 0;
         ...
         i++)
    {
        stddev_distance += std::pow(avgdistance - avgdistance_points[i],
                                    2);
    }
...
int Evolution::tournament()
{
    ...
    if (this->population->at(genome1)->getFitness() >
        this->population->at(genome2)->getFitness())
    {
...
void
Evolution::testGeneticString(int argc, char *argv[], std::string test_genome)
{
    ...
    if (myfile.is_open())
    {
        ...
        Genome *gen = new Genome(test_genome,
                                 ...
                                 -1,
                                 -1);
        gen->setGeneticString(gen->build_genetic_string(gen->getGeneticString(),
                                                        gs));
        ...
        gen->constructor(argc,
                         argv,
                         this->params,
                         "fixed");

        // measures all metrics od the genome
        Measures m(this->experiment_name,
                   this->params);
        m.setGenome(gen);
        m.measurePhenotype(this->params,
                           "fixed",
...
void Evolution::selection()
{
         ...
         i++)
    {
        ...
        while (std::find(index_selected.begin(),
                         index_selected.end(),
...
//    }
    ...
    this->tests.testPopsize(this->population,
...
void
Evolution::exportGenerationMetrics(int generation, std::vector<int> metrics)
{
         ...
         i++)
    {
        ...
        if (this->getPopulation()->at(i)->getFitness() >
            maximum_fitness)
...
void Evolution::setupEvolution()
{
    ...
    Aux aux = Aux(this->experiment_name,
                  this->getParams());
    this->aux = Aux(this->experiment_name,
                    this->getParams());
                                   ...
                                   std::string path)
{
    ...
    {
        // develops genome
        individuals->at(i)->developGenomeIndirect(
                argc,
                argv,
                this->params,
                LS,
...
void Evolution::loadPopulation(int generation){
    ...
    std::string pathdir =  "../../experiments/" + this->experiment_name + "/selectedpop"+ std::to_string(generation+1);
    system(("exec rm -r "+pathdir).c_str());
    pathdir =  "../../experiments/" + this->experiment_name + "/offspringpop"+ std::to_string(generation+1);
    system(("exec rm -r "+pathdir).c_str());

    // generates list of files (genomes of last population)
    std::system(("ls ../../experiments/" + this->experiment_name + "/selectedpop" +std::to_string(generation)+
                 ">../../experiments/" + this->experiment_name + "/temp.txt").c_str());

    std::ifstream listgenomes(("../../experiments/" + this->experiment_name + "/temp.txt").c_str());
    std::string linegenome;

    //for each file (genome)
    while ( getline (listgenomes, linegenome) ) {
        std::vector<std::string> tokens;
        boost::split(tokens, linegenome, boost::is_any_of("_."));
        ...
        while ( getline (listalphabet, linealphabet) ) {
            ...
            boost::split(items, linealphabet, boost::is_any_of(" "));
            ...
            gen->addLetterGrammar(items[0], lgs);
        ...
        while ( getline (listmeasures, linemeasures) ) {
            ...
            std::vector<std::string> tokens;
            boost::split(tokens, linemeasures, boost::is_any_of(":"));

            gen->updateMeasure(tokens[0], std::stod(tokens[1]));
        }
...
void Evolution::loadArchive()
{
    ...
    //for each file (genome)
    while (getline(listgenomes,
                   linegenome))
    {

        linegenome = linegenome.substr(6,
                                       linegenome.size());
        ...
        // recreates genome back to population
        // fitness of the parents is not loaded, but it doesnt matter, because at this point it has been saved to the history already
        Genome *gen = new Genome(idgenome,
                                 "N",
                                 "N",
                                 -1,
                                 -1);
                       ...
                       linealphabet))
        {
            ...
            boost::split(items,
                         linealphabet,
                         boost::is_any_of(" "));
            std::vector<std::string> items_rule(items.begin() + 1,
                                                items.begin() + items.size() -
                                                1);

            // build a genetic-string with the production rule for the letter
            GeneticString *lgs;
            lgs = gen->build_genetic_string(lgs,
                                            items_rule);

            // adds letter and its production rule (made a genetic-string) to the grammar of the genome
            gen->addLetterGrammar(items[0],
                                  lgs);
                       ...
                       linemeasures))
        {
            ...
            gen->updateMeasure(tokens[0],
                               std::stod(tokens[1]));
        }

        this->archive->emplace(idgenome,
...
int Evolution::loadExperiment()
{
                   ...
                   line))
    {
        ...
        boost::split(tokens2,
                     tokens[1],
                     boost::is_any_of(" "));
        std::vector<std::string> tokens3(tokens2.begin(),
                                         tokens2.begin() + tokens2.size() - 1);
...
int Evolution::initExperiment(int argc, char *argv[], LSystem LS)
{
    ...
    if (this->type_experiment == "novelty")
    {
        ...
        this->addToArchive(this->population,
...
double Evolution::runExperiment(int argc, char *argv[])
{
         ...
         g++)
    {
        ...
        // saves metrics of evolution to file
        this->exportGenerationMetrics(g,
                                      niche_measures);
...
Evolution::calculateNicheCoverage(std::vector<Genome *> *individuals)
{
         ...
         i++)
    {
        ...
        for (const auto &it : individuals->at(i)->getMeasures())
        {
            ...
            if (std::find(this->morphological_measures_accumulated[it.first].begin(),
        ...
        if (this->morphological_grid_accumulated.count(key_point) > 0)
        {
            ...
            this->morphological_grid_accumulated[key_point].push_back(individuals->at(i)->getId());
        ...
        else
        {
            ...
            individual.push_back(individuals->at(i)->getId());
