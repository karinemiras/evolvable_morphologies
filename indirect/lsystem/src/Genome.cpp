...
void Genome::setGrammar(std::map<std::string,
        GeneticString *> grammar)
{
                                 ...
                                 std::map<std::string, double> params)
{
    ...
    std::uniform_int_distribution<int> dist_2(1,
                                              (int) alp_i.size() - 1);
    // distribution for the mounting commands
    std::uniform_int_distribution<int> dist_3(0,
    ...
    GeneticString *gs;
    this->gs = this->build_genetic_string(gs,
                                          letter_items);
 ...
 */
void Genome::generate_final_string(int replacement_iterations,
                                   int export_genomes,
                                   int generation,
                                   std::string path)
{
 ...
 */
GeneticString *Genome::build_genetic_string(GeneticString *_gs,
                         ...
                         std::string path)
{
    ...
    if (params["export_phenotypes"] == 1)
    {
        ...
        QString qstr = QString::fromStdString("../../experiments/" +
                                              path + "/body_" +
                                              this->id + "_p1_" +
                            ...
                            std::map<std::string, double> params)
{
    ...
    { // condition to stop recursive calls
        ...
        if (c2->item == "PJ1")
        {
            items[items.size() - 1]->setBrush(QColor(128,
                                                     246,
                                                     152));  // light  green
        }
        if (c2->item == "PJ2")
        {
            items[items.size() - 1]->setBrush(QColor(34,
                                                     122,
                                                     27));  //  green dark
        }
        if (c2->item == "J1")
        {
            items[items.size() - 1]->setBrush(QColor(182,
                                                     186,
                                                     184)); // light grey
        }
        if (c2->item == "J2")
        {
            items[items.size() - 1]->setBrush(QColor(102,
                                                     105,
                                                     103));  // dark grey
        }
        if (c2->item == "AJ1")
        {
            items[items.size() - 1]->setBrush(QColor(233,
                                                     97,
                                                     118)); // light red
        }
        if (c2->item == "AJ2")
        ...
        scene->addItem(items[items.size() -
        ...
        {  // sets the component (and sign) at the proper position in the drawing
            ...
            { // if component is supposed to be mounted at the left-side of the parent
                ...
                { // if the back of the turtle is at the bottom-side of the screen
                    items[items.size() - 1]->setPos(c2->back->x - size - space,
                ...
                { // if the back of the turtle is at the lef- side of the screen
                    items[items.size() - 1]->setPos(c2->back->x,
        ...
        tsign = tsign
                + c2->sensor_left.substr(1,
                                         1)
                + c2->sensor_front.substr(1,
                                          1)
                + c2->sensor_right.substr(1,
                                          1);
        if (c2 == c1)
        {
            tsign = tsign + c2->sensor_back.substr(1,
                                                   1);
        }
        ...
        {  // saves shortcut for coordinates of the item
            ...
            std::pair<int, int> key = std::make_pair(c2->x,
                                                     c2->y);
            this->list_components[key] = c2->item;


            // writes the module into yaml file
            this->convertYamlBody(parent_convertion,
                                  _directoryPath,
                                  convertion_level,
                                  direction,
                                  c2,
        ...
        std::string sensor)
{
    ...
    if (letter_convertion.substr(0, 2) == "AJ")
    {
    ...
    if (sensor != "")
    {
        if(direction == "root-back")
            ...
            robot_file << "sensor-" << direction.substr(0, 1);
    }
